import { AttachmentBuilder, ButtonInteraction, EmbedBuilder, ModalSubmitInteraction } from "discord.js";
...
export default class Catch {
    ...
    static async catchModalSubmitted(btnInteraction: ButtonInteraction, modalInteraction: ModalSubmitInteraction, db: Database) {
        ...
        let encounter = await db.getEncounter(modalInteraction.guild.id, btnInteraction.channelId);
        if (encounter.length > 0) {
            ...
            await btnInteraction.followUp({
                embeds: [
                    Util.returnEmbed(lang.obj['catch_no_encounter_title'], lang.obj['catch_no_encounter_description']) as EmbedBuilder
                ],
                ephemeral: true
            });
            this.guessEntered = false // Reset guessEntered
