...
export default class Database {
    ...
    async addMod(mentionable: GuildMember | Role) {
        ...
        if (await this.isMod(mentionable)) {
            throw Util.isUser(mentionable) ? lang.obj['settings_mods_add_already_existing_user'].replace('{mentionable}', `<@!${mentionable.id}>`) : lang.obj['settings_mods_add_already_existing_role'].replace('{mentionable}', `<@&${mentionable.id}>`);
        } else {
    ...
    async removeMod(mentionable: GuildMember | Role): Promise<number> {
        const lang = await LanguageApi.getLanguage(mentionable.guild.id, this);
        if (!(await this.isMod(mentionable))) {
            throw Util.isUser(mentionable) ? lang.obj['settings_mods_remove_not_existing_mod_user'].replace('{mentionable}', `<@!${mentionable.id}>`) : lang.obj['settings_mods_remove_not_existing_mod_role'].replace('{mentionable}', `<@&${mentionable.id}>`);
        } else {
    ...
    async isAllowedChannel(channel: GuildChannel): Promise<boolean> {
        return (await this.Channel.count({
            where: {
                serverId: channel.guildId,
                channelId: channel.id
            }
    ...
    async setLanguage(serverId: string, languageCode: string) {
        ...
        if (await this.isLanguageSet(serverId, languageCode)) {
            ...
            return await this.Language.update({ languageCode }, {
                where: {
                    serverId
                }
            });
    ...
    async isLanguageSet(serverId: string, languageCode: string): Promise<boolean> {
        return (await this.Language.count({
            where: {
                serverId,
                languageCode
            }
    ...
    async getLanguageCode(serverId: string): Promise<string> {
        ...
        if (result.length < 1)
            ...
            return result[0].getDataValue('languageCode');
    }
    ...
    async removeScore(serverId: string, userId: string, score: number): Promise<number | [affectedCount: number]> {
        const lang = await LanguageApi.getLanguage(serverId, this);
        const current = await this.Score.findOne({
            where: {
                serverId,
    ...
    async addEncounter(serverId: string, channelId: string, name: string, language: string) {
    ...
    async artworkExists(serverId: string, channelId: string): Promise<boolean> {
        return (await this.Artwork.count({
            where: {
                serverId,
                channelId
            }
    ...
    async getArtwork(serverId: string, channelId: string): Promise<string | null> {
        return (await this.Artwork.findOne({
            where: {
                serverId,
                channelId
            }
    ...
    async unsetArtwork(serverId: string, channelId: string): Promise<number | null> {
        if (await this.artworkExists(serverId, channelId)) {
    ...
    async lastExploreExists(serverId: string, channelId: string): Promise<boolean> {
        return (await this.LastExplore.count({
            where: {
                serverId,
                channelId
            }
    ...
    async getLastExplore(serverId: string, channelId: string): Promise<number | null> {
        return (await this.LastExplore.findOne({
            where: {
                serverId,
                channelId
            }
    ...
    async unsetLastExplore(serverId: string, channelId: string): Promise<number | null> {
        if (await this.lastExploreExists(serverId, channelId)) {
