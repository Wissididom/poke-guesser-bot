  ...
  static async settings(
    ...
    if (!interaction.guild?.available) {
      await interaction.reply({
        content:
          "Guild not available  (settings -> settings -> interaction.guild.available is either null or false)",
        ephemeral: true,
      });
      return;
    }
    if (!interaction.guildId) {
      await interaction.reply({
        content:
          "Internal Server Error (settings -> settings -> interaction.guildId = null)",
        ephemeral: true,
      });
      return;
    }
